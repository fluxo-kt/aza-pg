...
**Multi-Stage Gosu Replacement: GHA Cache Ambiguity**: All multi-stage approaches (`COPY --from=`, bind-mounts, `apt-get install su-exec` — absent from postgres image repos, `COPY via builder-pgxs output dir` — COPY cache key matched stale GHA entry) fail due to GHA layer cache interference. **Definitive fix**: compile su-exec directly in the final stage in a single `RUN` (install build tools + clone + compile + purge). Install+purge in ONE `RUN` = net layer diff is only gosu→su-exec. First run is always fresh; subsequent builds cache the su-exec result. Add `[ SZ -lt 500000 ]` to FAIL build if wrong binary.
