...
- `scripts/release/promote-image.ts` - Digest-based image promotion with OCI metadata annotations
  ...
  - Extracts image metrics (size, layers) after promotion, outputs as GitHub Actions job outputs
...
- **auto_explain**: Module (shared_preload_libraries), NOT extension. NO CREATE EXTENSION needed (PostgreSQL design)
...
- `pgsodium`: Encryption library (⚠️ REQUIRED for event triggers to work; full TCE requires pgsodium_getkey script)
