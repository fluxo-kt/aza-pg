...
**Multi-Stage Gosu Replacement: GHA Cache Ambiguity**: `COPY --from=builder-stage`, `RUN --mount=type=bind,from=builder-stage,...`, and `apt-get install su-exec` (package absent from postgres image's stripped apt repos) all fail to reliably replace `/usr/local/bin/gosu`. **Definitive fix**: replace the binary inside `builder-pgxs` output dir (`/opt/ext-out/usr/local/bin/gosu`) using the pre-compiled binary from the same builder stage (`builder-pgxs` inherits from `builder-base` which compiled su-exec). `COPY --from=builder-pgxs /opt/ext-out/ /` then delivers the replaced binary to the final stage — no apt dependency, no bind-mount, no GHA cache ambiguity. Add `[ SZ -lt 500000 ]` assertion to FAIL the build if wrong binary.
