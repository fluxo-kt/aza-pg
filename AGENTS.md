...
**Multi-Stage Gosu Replacement: GHA Cache Ambiguity**: All multi-stage approaches (`COPY --from=`, bind-mounts, `apt-get install su-exec` — absent from postgres image repos, `COPY via builder-pgxs output dir` — COPY cache key matched stale GHA entry) fail due to GHA layer cache interference. **Critical gotcha**: replacing a file owned by a dpkg package (e.g., gosu) without purging the package first causes subsequent `apt-get install` steps with `--mount=type=cache,target=/var/cache/apt` to detect the hash mismatch and reinstate the original binary. **Definitive fix**: compile su-exec in the final stage → `apt-get purge gosu` (removes dpkg ownership) → install su-exec. Also exclude gosu from builder-pgxs rsync (`--exclude='gosu'`). Add `[ SZ -lt 500000 ]` to FAIL build if wrong binary.
