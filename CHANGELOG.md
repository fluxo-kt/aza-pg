...
- **gosu → su-exec: dpkg purge + self-contained compilation**: Five prior approaches failed — four due to GHA layer cache interference (`COPY --from=builder-base`, `RUN --mount=type=bind`, `apt-get install su-exec` — package absent, `COPY via builder-pgxs output dir` — COPY key matched stale GHA entry); the fifth compiled su-exec but apt reinstalled gosu in the next step (hash mismatch in dpkg database + persistent `/var/cache/apt` mount). Definitive fix: compile su-exec (v0.2, SHA-pinned) in the final stage → `apt-get purge gosu` (removes dpkg ownership, preventing apt reinstatement) → install su-exec at the same path. Also excludes gosu from builder-pgxs rsync to avoid redundant intermediate layers. Build-time assertion (`[ SZ -lt 500000 ]`) fails build if wrong binary. No future apt operation can reinstate gosu.
