...
- **gosu → su-exec: replacement in builder-pgxs output dir**: Three prior attempts failed — `COPY --from=builder-base`, `RUN --mount=type=bind,from=builder-base,...`, and `apt-get install su-exec` (package absent from postgres image's apt repos) all failed to reliably replace the binary. Final fix: replace `/opt/ext-out/usr/local/bin/gosu` with the pre-compiled su-exec binary inside `builder-pgxs` (which inherits su-exec from `builder-base`), so `COPY --from=builder-pgxs /opt/ext-out/ /` delivers su-exec at `/usr/local/bin/gosu` in the final stage. Build-time size assertion (`[ SZ -lt 500000 ]`) fails the build if the wrong binary is present (su-exec ~14KB, gosu ~7MB). No apt dependency, no bind-mount ambiguity.
