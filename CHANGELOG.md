...
- Eliminated `legacyTestFiles` in validate.ts: renamed `test-auto-config-units.ts` → `test-auto-config-units.test.ts` so all unit tests follow uniform `.test.ts` convention and are auto-discovered; renamed `test-security.test.ts` → `test-security.ts` (matching Docker-test naming convention), eliminating the `DOCKER_INTEGRATION_TESTS` exclusion set — Docker tests are now structurally excluded by naming alone (`test-*.ts` vs `*.test.ts`); `bun test` (no args) now runs only unit tests correctly; removed redundant explicit-file unit test step from ci.yml (covered by validate:all); `test:unit` package.json script now delegates to `bun run validate` (drift-proof); bunfig.toml comment corrects the record that `exclude` is NOT a supported option in Bun 1.3.x
- docs(TESTING.md): added "Test File Naming Convention" section explaining structural enforcement (`*.test.ts` vs `test-*.ts`); removed stale hardcoded test count (was 198, now 374)
- fix(test:all): test-all.ts had 3 explicit unit-test entries covering only 3/10 files; replaced with `bun test scripts` (directory-based, auto-discovers all 10 `*.test.ts` files — same invariant as validate.ts)
...
- fix(test-image-lib.test.ts): 18th kaizen — test 1 was missing expect() (inconsistent with test 2); added expect(mismatches.length).toBe(0) to match; consolidated all .so-module exclusions from mixed filter+continue into single SO_MODULE_TOOLS Set in filter with per-entry rationale comments (wal2json/pg_safeupdate/pg_plan_filter); removed redundant expect() after throw was itself a subtle zero-confidence repeat of the pattern just fixed
- fix(test-image-lib.ts): 19th kaizen — toolBinaries had `safeupdate` key (never matched manifest name `pg_safeupdate`) → pg_safeupdate's .so file was silently skipped in ALL Docker integration test runs; fixed key to `pg_safeupdate`; removed dead `plan_filter` entry (pg_plan_filter disabled/PG18-incompatible); updated unit test comment to clarify scope (CLI tools only — .so module paths excluded by design)
- fix(test-image-lib.ts): 20th kaizen — structural prevention: testToolsPresent() now filters enabled-only tools (disabled tools have no binaries to check) and fails loudly on unknown enabled tools instead of silently skipping them; adds comments documenting PG major version coupling in .so paths and key-must-match-manifest invariant
- docs(/update skill + AGENTS.md): 21st kaizen — version-check grep commands in /update skill were scoped to scripts/test/ only, missing scripts/docker/ entirely; broadened to scripts/; added specific check for hardcoded PG major lib paths (postgresql/N/lib); added PG MAJOR bump checklist; added test-image-lib.ts toolBinaries invariants to mandatory doc sync and AGENTS.md Key Learnings
- fix(/update skill): 22nd kaizen — 10 grep commands used `\|`/`\+` (GNU BRE extensions) that silently produce zero output on macOS BSD grep; fixed all to use `-E` (POSIX ERE) where `|` and `+` work portably; also fixed backtick boundary bug on Phase 9.5 "Search all test files" line (pipe was prose text, not part of the command); fixed unescaped `.bun/` → `\.bun/` in all grep -v patterns
- fix(test-image-lib): 24th kaizen — 8 false-confidence bugs from adversarial audit: testPostgresConfiguration replaced vacuous `val.length > 0` with `pg_size_bytes()` threshold checks (>=16MB shared_buffers, >=1MB work_mem, >=20 max_connections); testAutoConfigApplied added `pg_settings.source` check (source='default' means auto-config entrypoint did not run); testVectorscaleDiskann added `SET enable_seqscan=OFF` (same planner bypass issue as HNSW, missed in pass 23); testPgStatStatements now executes a tracked query after reset and asserts count>=1 (previously counted 0 rows and still passed); testPgTrgmSimilarity asserts exact expected row "hello world" instead of any non-empty string; testWal2jsonReplication validates changes.output is non-empty (success + empty = slot created but capture broken); testRumRankedSearch adds `<=>` RUM-specific operator test (@@ works with GIN too — RUM index never actually tested before); testTimescaledbHypertables verifies chunk count>=1 via timescaledb_information.chunks (the defining hypertable feature was entirely untested)
- fix(test-image-lib): 23rd kaizen — 8 bugs found by adversarial audit: quoted extension names in CREATE EXTENSION statements (lines 462/593 used unquoted identifiers, inconsistent with testPrecreatedExtensions); HNSW search test added `SET enable_seqscan = OFF` (planner selects SeqScan on 3-row table, bypassing HNSW index, producing false confidence); pg_safeupdate added DELETE-without-WHERE protection test (only UPDATE was covered, DELETE was entirely untested); wal2json added slot cleanup in catch block (slot leaked when exception fired after slot creation but before try block's cleanup ran); corrected pgsodium comment (falsely claimed pg_partman CASCADE dependency — actual issue is DDL event trigger interference); cleanupTestData removed stale entries `test_btree_gin`/`test_btree_gist` (tables never created, btree_gist test creates `test_exclusion`) and added `pgmq.drop_queue('test_queue')` (pgmq queues live in pgmq schema, not cleaned by DROP TABLE); /update skill Phase 3 docker inspect SHA strip (`sed 's/.*@//'` — inspect returns full `docker.io/library/postgres@sha256:X` path, manifest needs just `sha256:X`); fixed pgflow duplicate Step 4 (was identical to Step 2, renumbered 5→6→7)
