...
- Eliminated `legacyTestFiles` in validate.ts: renamed `test-auto-config-units.ts` → `test-auto-config-units.test.ts` so it joins the auto-discovered glob (all unit tests now follow uniform `.test.ts` convention); added `exclude` to `bunfig.toml [test]` for structural Docker-test exclusion when running bare `bun test`; test-all.ts and package.json updated to match
