...
- **gosu → su-exec: multi-stage dependency eliminated**: Two prior attempts failed due to GHA cache ambiguity — `COPY --from=builder-base` and `RUN --mount=type=bind,from=builder-base,...` both left the original gosu binary detectable by Trivy. Root cause: the bind-mount source resolution against a GHA-cached builder-base cannot be verified at build time. Final fix: install `su-exec` directly from Debian Trixie apt in the final stage (no multi-stage dependency), then replace `/usr/local/bin/gosu` with a build-time size assertion (`[ SZ -lt 500000 ]`) that fails the build if the replacement produces the wrong binary (gosu ~7MB, su-exec ~14KB).
