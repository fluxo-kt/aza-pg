...
- **gosu → su-exec: self-contained final-stage compilation**: Four prior approaches failed due to GHA layer cache interference (`COPY --from=builder-base`, `RUN --mount=type=bind`, `apt-get install su-exec` — package absent, `COPY via builder-pgxs output dir` — COPY cache key matched stale GHA entry predating the replacement). Definitive fix: compile su-exec (v0.2, SHA-pinned) directly in the final stage (`build-essential git` install + compile + purge, all in one `RUN`). Net layer diff = only gosu→su-exec (~14KB); build tools leave no trace. Build-time assertion (`[ SZ -lt 500000 ]`) fails build if replacement produces wrong binary. Guaranteed cache-independent: first run always compiles fresh, subsequent runs use cached su-exec result.
