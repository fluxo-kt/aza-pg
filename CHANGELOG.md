...
- **gosu → su-exec cache bypass**: The initial `COPY --from=builder-base` implementation of the gosu replacement was silently defeated by a stale BuildKit registry cache entry that served the original Go binary. Changed to `RUN --mount=type=bind,from=builder-base,...` which has stronger cache invalidation semantics tied to the full parent-layer chain, making the su-exec replacement deterministic.
