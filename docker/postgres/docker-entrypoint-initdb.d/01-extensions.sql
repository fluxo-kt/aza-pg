...
CREATE EXTENSION IF NOT EXISTS "pgflow"; -- pgflow (workflow)
...
DO $$
BEGIN
  RAISE NOTICE 'Baseline extensions enabled (pg_cron, pg_stat_monitor, pg_stat_statements, pg_trgm, pgaudit, pgflow, plpgsql, vector, vectorscale). Additional extensions are available but disabled by default.';
END;
$$;
