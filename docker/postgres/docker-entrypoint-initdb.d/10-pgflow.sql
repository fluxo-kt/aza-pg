-- ============================================================================
-- pgflow v0.7.2 - SQL Schema for Standalone PostgreSQL
-- ============================================================================
--
-- This is a consolidated schema extracted from pgflow's Supabase migrations.
-- Source: github.com/pgflow/pgflow @ @pgflow/core@0.7.2
--
-- DEPENDENCIES:
--   - pgmq extension (PostgreSQL Message Queue)
--   - PostgreSQL 14+ (for gen_random_uuid, jsonb functions)
--
-- LIMITATIONS WITHOUT SUPABASE:
--   - No real-time event broadcasting (realtime.send calls stubbed)
--   - No Row Level Security / auth.users integration
--   - You must implement your own authentication/authorization layer
--
-- INSTALLATION ORDER:
--   1. CREATE EXTENSION pgmq;
--   2. Run this file
--   3. Configure your application to use pgflow functions
--
-- ============================================================================
...
COMMENT ON FUNCTION realtime.send IS
  'Stub function replacing Supabase realtime.send(). Replace with your own event notification system.';
...
CREATE FUNCTION "pgflow"."poll_for_tasks"(
  ...
  "max_poll_seconds" integer DEFAULT 5,
  "poll_interval_ms" integer DEFAULT 100
) RETURNS SETOF "pgflow"."step_task_record" LANGUAGE sql SET "search_path" = '' AS $$
...
) RETURNS "pgflow"."steps" LANGUAGE sql SET "search_path" = '' AS $$
WITH
  next_index AS (
...
) RETURNS SETOF "pgflow"."step_tasks" LANGUAGE plpgsql SET "search_path" = '' AS $$
BEGIN
...
) RETURNS "pgflow"."flows" LANGUAGE sql SET "search_path" = '' AS $$
WITH
  flow_upsert AS (
...
) RETURNS SETOF "pgflow"."runs" LANGUAGE plpgsql SET "search_path" = '' AS $$
DECLARE
...
) RETURNS SETOF "pgflow"."step_task_record" LANGUAGE plpgsql SET "search_path" = '' AS $$
DECLARE
  msg_ids bigint[];
BEGIN
