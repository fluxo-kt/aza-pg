...
The aza-pg image includes 38 total catalog entries (36 enabled extensions, 2 disabled: pgq, supautils). You can reduce image size and build time by disabling unused extensions via the manifest-driven system. See [EXTENSIONS.md](EXTENSIONS.md#enabling-and-disabling-extensions) for step-by-step instructions.
