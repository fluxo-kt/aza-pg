...
function generatePgdgPackagesInstall(_manifest: Manifest): string {
  ...
  const shellScript = `RUN set -eu && \\
    ...
    add_if_enabled() { \\
      ...
      local pkg_name=$2; \\
      local pkg_version=$3; \\
      if jq -e --arg name "$ext_name" '.entries[] | select(.name == $name and .install_via == "pgdg" and (.enabled // true) == true)' /tmp/extensions.manifest.json >/dev/null; then \\
  ...
  const finalPart = `    # Install only enabled packages (skip if none enabled)
    if [ -n "$PGDG_PACKAGES" ]; then \\
      ...
      ENABLED_PGDG_COUNT=$(jq '[.entries[] | select(.install_via == "pgdg" and .enabled == true)] | length' /tmp/extensions.manifest.json) && \\
...
function generateVersionInfoGeneration(_manifest: Manifest): string {
  ...
  return `RUN set -ex; \\
    ...
    ENABLED=$(jq '[.entries[] | select(.enabled // true)] | length' "$MANIFEST"); \\
    DISABLED=$(jq '[.entries[] | select(.enabled // true | not)] | length' "$MANIFEST"); \\
    PRELOADED=$(jq '[.entries[] | select((.enabled // true) and .runtime.sharedPreload)] | length' "$MANIFEST"); \\
    \\
