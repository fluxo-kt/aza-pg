...
/**
 * Test 14: 12GB RAM, 8 vCPU (I/O workers threshold)
 */
async function case12gb8cpu(logs: string, container: string): Promise<void> {
  assertLogContains(logs, "RAM: 1228[0-9]MB \\(cgroup-v2\\)", "Detected 12GB via cgroup");
  assertLogContains(logs, "CPU: 8 cores", "CPU detection picked up 8 cores");
  assertLogContains(logs, "shared_buffers=2457MB", "shared_buffers tuned to 20% for 12GB");
  assertLogContains(logs, "io_workers=3", "I/O workers set to 3 (8 cores / 4, threshold met)");
...
async function main(): Promise<void> {
  ...
  await runCase(
    "\n📌 Test 14: 12GB RAM, 8 vCPU (I/O workers threshold)",
    case12gb8cpu,
    ["-m", "12g", "--cpus=8", "-e", `POSTGRES_PASSWORD=${testPassword}`],
    imageTag
  );
