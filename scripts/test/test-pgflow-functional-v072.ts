 ...
 *   bun run scripts/test/test-pgflow-functional-v072.ts --image=aza-pg:latest
 *   bun run scripts/test/test-pgflow-functional-v072.ts --image=ghcr.io/fluxo-kt/aza-pg:18
 *
 *   # Use existing running container
 *   bun run scripts/test/test-pgflow-functional-v072.ts --container=my-postgres
 *   bun run scripts/test/test-pgflow-functional-v072.ts --container=primary-postgres-primary --database=pgflow_test
 *
 * Parameters:
 *   --image=TAG              Docker image to test (starts new container, cleans up on exit)
...
// Determine mode: --image (start new container) or --container (use existing)
const useExistingContainer = Boolean(existingContainer);
const CONTAINER = useExistingContainer
  ? existingContainer!
  : `test-pgflow-v072-${Date.now()}-${process.pid}`;
...
console.log(
  useExistingContainer
    ...
    : `Starting new container from image: ${imageTag}`
);
...
/**
 * Start container from image (only if --image mode)
 */
async function startContainer(): Promise<void> {
  if (useExistingContainer) {
    return; // Skip if using existing container
  }
  ...
  try {
    ...
    throw new Error(`Container ${CONTAINER} not ready after ${timeout / 1000}s`);
  } catch (error) {
...
/**
 * Cleanup container (only if we started it)
 */
async function cleanup(): Promise<void> {
  if (!containerStarted || useExistingContainer) {
    return; // Skip if using existing container or never started
  }
...
async function main() {
  ...
  `);
      ...
      const verify = await runSQL(
        `SELECT count(*) FROM pgflow.flows WHERE flow_slug = '${FLOW_SLUG}'`
      );
  ...
  `);
    ...
    await test("Cleanup test data", async () => {
      ...
      const verify = await runSQL(
        `SELECT count(*) FROM pgflow.flows WHERE flow_slug LIKE 'test_%'`
      );
      assert(
    ...
    // Always cleanup if we started a container
    await cleanup();
  }
}
