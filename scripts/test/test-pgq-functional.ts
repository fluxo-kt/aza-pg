...
await test("Queue creation and configuration", async () => {
  ...
  assert(info.success && info.stdout === 'test_queue_functional', "Queue not found in get_queue_info");
  ...
  const verifyConfig = await runSQL("SELECT queue_ticker_max_count FROM pgq.get_queue_info('test_queue_functional')");
  assert(verifyConfig.success && verifyConfig.stdout === '10', "Queue configuration verification failed");
});

// Test 2: Producer Operations - Insert Events
await test("Producer operations - insert events", async () => {
  // Insert simple event
  const event1 = await runSQL("SELECT pgq.insert_event('test_queue_functional', 'user.created', '{\"user_id\": 1, \"email\": \"test@example.com\"}')");
  assert(event1.success && parseInt(event1.stdout) > 0, "Simple event insertion failed");

  // Insert event with extra fields
  const event2 = await runSQL("SELECT pgq.insert_event('test_queue_functional', 'user.updated', '{\"user_id\": 1}', 'extra1', 'extra2', 'extra3', 'extra4')");
  ...
  for (let i = 0; i < 10; i++) {
    const eventN = await runSQL(`SELECT pgq.insert_event('test_queue_functional', 'order.created', '{"order_id": ${i + 1}}')`);
...
await test("Consumer registration", async () => {
  ...
  const register = await runSQL("SELECT pgq.register_consumer('test_queue_functional', 'test_consumer')");
  assert(register.success && register.stdout === '1', "Consumer registration failed");

  // Verify consumer exists
  const consumerInfo = await runSQL("SELECT consumer_name FROM pgq.get_consumer_info('test_queue_functional', 'test_consumer')");
  assert(consumerInfo.success && consumerInfo.stdout === 'test_consumer', "Consumer not found in get_consumer_info");
});
...
await test("Ticker - create tick for event processing", async () => {
  ...
  const tickCheck = await runSQL("SELECT count(*) > 0 FROM pgq.get_queue_info('test_queue_functional') WHERE last_tick_id > 0");
...
await test("Batch processing - get next batch", async () => {
  ...
  // Get batch info
  const batchInfo = await runSQL(`SELECT batch_start IS NOT NULL FROM pgq.get_batch_info(${batchId})`);
  ...
  assert(events.success && parseInt(events.stdout) >= 12, `Expected at least 12 events, got ${events.stdout}`);

  // Process events (simulate work)
  const eventDetails = await runSQL(`SELECT ev_type, ev_data FROM pgq.get_batch_events(${batchId}) ORDER BY ev_id LIMIT 3`);
...
await test("Event retry logic", async () => {
  ...
  const event = await runSQL("SELECT pgq.insert_event('test_queue_functional', 'order.failed', '{\"order_id\": 999}')");
  ...
  // Get an event ID
  const eventId = await runSQL(`SELECT ev_id FROM pgq.get_batch_events(${batchId}) WHERE ev_type = 'order.failed' LIMIT 1`);
  assert(eventId.success && eventId.stdout, "Getting event ID for retry failed");
...
await test("Batch retry logic", async () => {
  // Insert events
  for (let i = 0; i < 3; i++) {
    await runSQL(`SELECT pgq.insert_event('test_queue_functional', 'bulk.retry', '{"item": ${i + 1}}')`);
  }
  ...
  assert(retry.success && parseInt(retry.stdout) >= 3, `Expected at least 3 events retried, got ${retry.stdout}`);
});

// Test 8: Queue Monitoring
await test("Queue monitoring", async () => {
  // Get queue info
  const queueInfo = await runSQL("SELECT queue_name, queue_ntables, queue_cur_table, ev_per_sec FROM pgq.get_queue_info('test_queue_functional')");
  assert(queueInfo.success && queueInfo.stdout.includes('test_queue_functional'), "Queue info retrieval failed");

  // Get consumer info
  const consumerInfo = await runSQL("SELECT consumer_name, lag, pending_events FROM pgq.get_consumer_info('test_queue_functional', 'test_consumer')");
  assert(consumerInfo.success && consumerInfo.stdout.includes('test_consumer'), "Consumer info retrieval failed");
...
await test("Consumer unregistration and cleanup", async () => {
  // Unregister consumer
  const unregister = await runSQL("SELECT pgq.unregister_consumer('test_queue_functional', 'test_consumer')");
  assert(unregister.success && unregister.stdout === '1', "Consumer unregistration failed");

  // Verify consumer removed
  const consumerCheck = await runSQL("SELECT count(*) FROM pgq.get_consumer_info('test_queue_functional', 'test_consumer')");
  assert(consumerCheck.success && consumerCheck.stdout === '0', "Consumer still exists after unregistration");
  ...
  const queueCheck = await runSQL("SELECT count(*) FROM pgq.get_queue_info('test_queue_functional')");
...
await test("Performance benchmark - event throughput", async () => {
  ...
  console.log(`   📊 Throughput: ${throughput.toFixed(2)} events/sec (${eventCount} events in ${insertDuration}ms)`);
...
await test("Concurrent consumer processing", async () => {
  ...
  // Insert events
  for (let i = 0; i < 20; i++) {
    await runSQL(`SELECT pgq.insert_event('concurrent_queue', 'concurrent.test', '{"id": ${i + 1}}')`);
  }
  ...
  assert(count1.stdout === count2.stdout, `Event counts don't match: ${count1.stdout} vs ${count2.stdout}`);
