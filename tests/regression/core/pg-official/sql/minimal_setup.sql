--
-- MINIMAL_SETUP --- prepare minimal environment for basic regression tests
--
-- This is a subset of test_setup.sql that creates tables using INSERT
-- statements only (no COPY from external files, no regress.so C functions).
-- Enables: int2, int4, int8, float8, text, varchar tests
--
-- NOTE: datestyle, timezone, intervalstyle are set via PGOPTIONS in the
-- test runner to match pg_regress expectations.
--
...
--
-- SQL hash functions for partition tests (no C library required)
--
