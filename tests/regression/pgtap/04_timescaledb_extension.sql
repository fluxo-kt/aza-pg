...
-- Verify extension
SELECT has_extension('timescaledb', 'timescaledb extension should be installed');
...
SELECT has_table('public', 'test_metrics', 'test_metrics table should exist');

-- Convert to hypertable
SELECT create_hypertable('test_metrics', 'time');

SELECT ok(
    (SELECT COUNT(*) FROM timescaledb_information.hypertables WHERE hypertable_name = 'test_metrics') = 1,
    'test_metrics should be a hypertable'
);

-- Insert test data
INSERT INTO test_metrics (time, device_id, temperature, humidity) VALUES
...
SELECT is(
    (SELECT COUNT(*)::INTEGER FROM test_metrics),
    4,
...
SELECT has_view('public', 'test_metrics_hourly', 'continuous aggregate view should exist');

SELECT ok(
    (SELECT COUNT(*) FROM timescaledb_information.continuous_aggregates
     WHERE view_name = 'test_metrics_hourly') = 1,
    'test_metrics_hourly should be a continuous aggregate'
);

-- Test data retention policy
SELECT add_retention_policy('test_metrics', INTERVAL '90 days');

SELECT ok(
    (SELECT COUNT(*) FROM timescaledb_information.jobs
     WHERE proc_name = 'policy_retention') >= 1,
...
SELECT ok(
    (SELECT COUNT(*) FROM timescaledb_information.jobs
     WHERE proc_name = 'policy_compression') >= 1,
...
SELECT ok(
    (SELECT COUNT(*) FROM timescaledb_information.chunks
     WHERE hypertable_name = 'test_metrics') >= 1,
    'hypertable should have at least one chunk'
);

-- Test approximate row count
SELECT ok(
    (SELECT approximate_row_count('test_metrics')::INTEGER) >= 4,
    'approximate_row_count should return >= 4'
);

-- Test first/last aggregates
SELECT ok(
    (SELECT first(temperature, time) FROM test_metrics) IS NOT NULL,
    'first() aggregate should work'
);

SELECT ok(
    (SELECT last(temperature, time) FROM test_metrics) IS NOT NULL,
