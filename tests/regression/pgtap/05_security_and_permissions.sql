...
-- Test default roles
SELECT has_role('postgres', 'postgres role should exist');
SELECT is_superuser('postgres', 'postgres should be superuser');

-- Create test role
CREATE ROLE test_user WITH LOGIN PASSWORD 'test_password';
SELECT has_role('test_user', 'test_user role should be created');
SELECT isnt_superuser('test_user', 'test_user should not be superuser');

-- Test schema permissions
CREATE SCHEMA IF NOT EXISTS test_security;
SELECT has_schema('test_security', 'test_security schema should exist');
...
CREATE TABLE test_security.sensitive_data (
    id SERIAL PRIMARY KEY,
    data TEXT NOT NULL
);

SELECT has_table('test_security', 'sensitive_data', 'sensitive_data table should exist');

GRANT SELECT ON test_security.sensitive_data TO test_user;
...
SELECT ok(
    (SELECT rowsecurity FROM pg_tables
     WHERE schemaname = 'test_security' AND tablename = 'sensitive_data'),
    'RLS should be enabled on sensitive_data'
);

-- Create RLS policy
CREATE POLICY user_data_policy ON test_security.sensitive_data
    FOR SELECT
    TO test_user
    USING (current_user = 'test_user');

SELECT ok(
    (SELECT COUNT(*) FROM pg_policies
     WHERE schemaname = 'test_security'
       AND tablename = 'sensitive_data'
       AND policyname = 'user_data_policy') = 1,
    'RLS policy should be created'
);

-- Test function security
CREATE OR REPLACE FUNCTION test_security.secure_function()
RETURNS TEXT
SECURITY DEFINER
...
SELECT has_function('test_security', 'secure_function', 'secure_function should exist');
...
SELECT ok(
    current_setting('ssl')::boolean IS NOT NULL,
    'SSL setting should be defined'
);
...
SELECT is(
    (SELECT rolconnlimit FROM pg_roles WHERE rolname = 'test_user'),
    10,
...
SELECT ok(
    (SELECT rolvaliduntil FROM pg_roles WHERE rolname = 'test_user') IS NOT NULL,
...
SELECT CASE
    ...
    THEN has_role('pg_read_all_data', 'pg_read_all_data role should exist')
...
SELECT CASE
    WHEN current_setting('server_version_num')::int >= 140000
    THEN has_role('pg_write_all_data', 'pg_write_all_data role should exist')
...
SELECT ok(
    (SELECT COUNT(*) FROM pg_default_acl
     WHERE defaclnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'test_security')) >= 1,
